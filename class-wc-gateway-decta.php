	...
	class WC_Decta_Gateway extends WC_Payment_Gateway {
	    ...
	    public function init_form_fields()
	    {
			...
			$this->form_fields = array(
				...
				'default'     	=> 'yes',
				'description' 	=> sprintf( __( 'Log DectaGateway events, inside <code>%s</code>', 'woocommerce' ), wc_get_log_file_path( 'DectaGateway' ) )
				)
			);
	    }
		...
		public function process_payment( $order_id ) {			
			...
			$this->addProducts($decta, $order, $params);
			$this->addCarrier($decta, $order, $params);

			$decta->log_info("Params for payment: " . print_r($params['products'], true));
		...
		protected function addProducts($decta, $order, &$params) {
			$params['products'] = [];
			$products = $order->get_items();

            foreach ($products as $product) {
				$data = $product->get_data();
				$regular_price = get_post_meta( $product['product_id'], '_regular_price', true);
				$sale_price = get_post_meta( $product['product_id'], '_price', true);
				$discount_amount = $regular_price - $sale_price;

				$price_without_tax = $data['subtotal'] / $data['quantity'];
				$price = ($discount_amount > 0) ? $regular_price : $price_without_tax;

				$tax_percent = ($product['subtotal_tax'] / $data['quantity']) * 100 / $price_without_tax;
				...
				if ($discount_amount > 0) {
					$fields['discount_amount'] = round($discount_amount * $data['quantity'], 2);
				}
				
				if ($tax_percent > 0) {
					$fields['tax_percent'] = round($tax_percent, 2);
				}
				$params['products'][] = $fields;
			}
		}

		protected function addCarrier($decta, $order, &$params) {
			if ($order->get_shipping_total() > 0) {
				$shipping_price = $order->get_shipping_total(); 
				$tax_percent = $order->get_shipping_tax() * 100 / $shipping_price;
				$params['products'][] = [
